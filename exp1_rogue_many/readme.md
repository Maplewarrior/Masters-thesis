# Experiment 1: Rogue Many
This is an extension of [Experiment 1: Rogue One](../exp1_rogue_one/readme.md) where instead of a single rogue point, we have multiple rogue points in a cluster with the same label and the rogue point from the previous experiment as the centroid of the cluster.
...
```bash
python exp1_rogue_many/generate_data.py
```

To run the experiment for all the datasets and unlearning methods, run:
```bash
python exp1_rogue_many/experiment_run.py data.dataset="data/data_1.npz,data/data_2.npz,data/data_3.npz,data/data_4.npz,data/data_5.npz" unlearn.method="amnesiac,ssd,retrain,scrubr,sae,sisa" hydra/launcher=ray --multirun
```
