...
class SelectiveSynapticDampening(SSD):
    ...
    def update_parameters(self, FIM_full, FIM_forget, return_dampening: bool = False, **kwargs):
        all_dampenings = {}
    ...
    def search_hyperparameters_TPE(self,
        ...
        def objective(trial):
            params = {f'alpha_{i}': trial.suggest_float(f"alpha_{i}", 0.1, 1000, log=False) for i in range(self.P)}
            params.update({f'_lambda_{i}': trial.suggest_float(f"_lambda_{i}", 0.1, 100., log=False) for i in range(self.P)})
    ...
    def __call__(self, 
        ...
        all_dampenings = self.update_parameters(FIM_full, FIM_forget, **opt_result['max']['params'], return_dampening=return_dampening)
        # print(f'SD identical? {self.check_statedict_equivalent(sd_original, self.model.state_dict())}')
        return opt_result, all_dampenings