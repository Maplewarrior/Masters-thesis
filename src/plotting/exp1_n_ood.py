...
def print_dict_structure(d, indent=0, max_depth=3, current_depth=0, max_items=5, file=None):
    """
    Print a summary of a nested dictionary structure, limiting depth and items per level.
    
    Args:
        d: Dictionary to summarize
        ...
        if isinstance(d, list):
            if len(d) > 0:
                if isinstance(d[0], dict):
                    print(" " * indent + f"List[{len(d)}] of dicts:", file=file)
                    if len(d) > 0 and current_depth < max_depth:
                        print_dict_structure(d[0], indent + 4, max_depth, current_depth + 1, max_items, file)
                else:
                    print(" " * indent + f"List[{len(d)}] of {type(d[0]).__name__}", file=file)
            else:
                print(" " * indent + "Empty list", file=file)
        else:
            print(" " * indent + f"{type(d).__name__}: {d}", file=file)
        return
    
    for i, (key, value) in enumerate(d.items()):
        if i >= max_items:
            print(" " * indent + f"... ({len(d) - max_items} more keys)", file=file)
            break
            
        if isinstance(value, dict):
            print(" " * indent + f"{key}: dict with {len(value)} keys", file=file)
            print_dict_structure(value, indent + 4, max_depth, current_depth + 1, max_items, file)
        elif isinstance(value, list):
            if len(value) > 0:
                if isinstance(value[0], dict):
                    print(" " * indent + f"{key}: List[{len(value)}] of dicts", file=file)
                    if current_depth < max_depth and len(value) > 0:
                        print_dict_structure(value[0], indent + 4, max_depth, current_depth + 1, max_items, file)
                else:
                    print(" " * indent + f"{key}: List[{len(value)}] of {type(value[0]).__name__}", file=file)
            else:
                print(" " * indent + f"{key}: Empty list", file=file)
        else:
            # For simple values, print the type and a preview
            value_str = str(value)
            if len(value_str) > 50:
                value_str = value_str[:47] + "..."
            print(" " * indent + f"{key}: {type(value).__name__} = {value_str}", file=file)

# Example usage:
# results = {...}  # Your dictionary
# print_dict_structure(results)

# To save to a file:
# with open("dict_structure.txt", "w") as f:
#     print_dict_structure(results, file=f)

def plot_performance_metrics(results: Dict[str, Dict[str, Any]], 
                             colors: Dict[str, str] = None, 
                             save_to_pdf: bool = False, 
                             save_to_png: bool = False, 
                             experiments_folder: str = "results/exp1_n_ood"):
    """
    ...
    if save_to_pdf:
        print(f"Saving memory usage metrics to {experiments_folder}/memory_usage_metrics_grid.pdf")
...
def _plot_performance_grid(results, metrics, colors, title, ylabel):
    ...
    for i, metric in enumerate(metrics):
        ...
        for unlearn_type, unlearn_data in results.items():
            # Extract x values (OOD ratios) and sort them
            x_values = sorted([float(x) for x in unlearn_data.keys()])
            ...
            for x in x_values:
                x_str = str(x)
                if x_str in unlearn_data and metric in unlearn_data[x_str]:
                    y_values.append(unlearn_data[x_str][metric]['mean'])
                    y_errors.append(unlearn_data[x_str][metric]['std'])
                else:
                    # Handle missing data
...
def plot_accuracy_metrics(results: Dict[str, Dict[str, Any]], colors: Dict[str, str] = None, save_to_pdf: bool = False, save_to_png: bool = False, experiments_folder: str = "results/exp1_n_ood"):
    """
...
def plot_equiv_metrics(results: Dict[str, Dict[str, Any]], colors: Dict[str, str] = None, save_to_pdf: bool = False, save_to_png: bool = False, experiments_folder: str = "results/exp1_n_ood"):
    """
...
def _plot_metric_grid(results, metrics, colors, title, ylabel):
    ...
    # Add buffers to the maximum and minimum values
    y_max_limit = max_y_value + 0.1
    y_min_limit = max(0, min_y_value - 0.1)  # Ensure we don't go below 0 for most metrics
...
def plot(results: Dict[str, Dict[str, Any]], save_to_pdf: bool = False, save_to_png: bool = False, experiments_folder: str = "results/exp1_n_ood"):
    """
...
if __name__ == "__main__":
    ...
    experiments_folder = "results/exp1_n_ood"
    ...
    if args.mode == "data":
        ...
        from wandb_client import plot_grid
        # plot the data
